...
const CONFIG = {
  // Startup modes: 'normal', 'debug', 'agents-only', 'monitor-only'
  STARTUP_MODE: process.env.STARTUP_MODE || 'normal',
  
  // Processing controls
  AUTO_START_AGENTS: process.env.AUTO_START_AGENTS === 'true',
  AUTO_START_PIPELINE: process.env.AUTO_START_PIPELINE === 'true',
  ...
  // Workflow presets
  WORKFLOW_PRESET: process.env.WORKFLOW_PRESET || 'thorough', // 'thorough', 'debug'
...
console.log('🎛️ Orchestrator Configuration:', {
  ...
  auto_start_agents: CONFIG.AUTO_START_AGENTS,
  auto_start_pipeline: CONFIG.AUTO_START_PIPELINE,
...
const supabase = createClient(
  ...
  process.env.SUPABASE_SERVICE_KEY || 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImFheXZ2Y3B4YWZ6aGNqcWV3d2phIiwicm9sZSI6InNlcnZpY2Vfcm9sZSIsImlhdCI6MTc0NjQ2Nzc1NCwiZXhwIjoyMDYyMDQzNzU0fQ.PHNLmAb0-jzy0CGl3ThVdgXZkAGTBWLxC5O-RDgp_yQ'
);
...
// Agent lifecycle management functions (Docker Container Compatible)
...
async function stopAgent(agentType) {
  console.log(`🛑 Releasing ${agentType} agent container tracking...`);
  
  const agentInfo = agentProcesses[agentType];
  if (agentInfo) {
    // In containerized environment, we don't stop containers - just remove from tracking
    delete agentProcesses[agentType];
    console.log(`✅ ${agentType} agent tracking removed (container continues running)`);
  }
}
...
async function checkAgentHealth(agentType) {
  const agent = AGENTS[agentType];
  // Use Docker container service names instead of localhost
...
app.post('/kill-agent-ports', async (req, res) => {
  try {
    ...
    const agentPorts = [3001, 3002, 3003, 3004, 3005];
    const killResults = [];
