...
# Check if .env exists in infrastructure/configs directory for building
ENV_FILE="$PROJECT_ROOT/infrastructure/configs/.env"
if [ -f "$ENV_FILE" ]; then
    ...
    echo "   To enable: copy infrastructure/configs/.env.example to infrastructure/configs/.env and add your credentials"
fi
