...
if [ ! -f "$PROJECT_ROOT/infrastructure/configs/.env" ]; then
    cp "$PROJECT_ROOT/infrastructure/configs/.env.example" "$PROJECT_ROOT/infrastructure/configs/.env"
    echo "⚠️  Created .env file at $PROJECT_ROOT/infrastructure/configs/.env"
    echo "   Please edit it with your API keys!"
fi

# Add alias to .zshrc
ALIAS_LINE="alias claude-docker='$PROJECT_ROOT/tools/scripts/claude-docker.sh'"
...
chmod +x "$PROJECT_ROOT/tools/scripts/claude-docker.sh"
chmod +x "$PROJECT_ROOT/tools/scripts/startup.sh"

echo ""
echo "Installation complete! 🎉"
echo ""
echo "Next steps:"
echo "1. (Optional) Edit $PROJECT_ROOT/infrastructure/configs/.env with your API keys"
